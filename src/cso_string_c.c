...
cso_string_c cso_string_new(const char* string)
{
    ...
    cso_object_initializer(cso_super_cast(cso_object_c, self), cso_toTypeNameStr(cso_string_c), sizeof(cso_string_t));
    return self;
}
...
{
    if (!self) return NULL;
    if (cso_get_obj_metadata(self).moveable) return self;
    if(!cso_instanceof(self, cso_string_c)) return NULL;
